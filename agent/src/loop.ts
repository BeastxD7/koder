...
export async function runPrompt(
   ...
   * Deliberately NOT stored on `AgentSession` — sessions are reused across
   * turns/prompts (server.ts keeps one `Session` per ACP session for its
   * whole lifetime), so depth must be threaded per-call, not attached to
