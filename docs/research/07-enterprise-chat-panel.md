...
## P0 (parity floor)
1. ✅ Chat store + restore-on-show + history panel (extension-owned transcript → ~/.lakshx/chats/*.json)
2. Auto-generated titles — `lakshx/title` runtime request w/ cheapest model of session provider; fallback first-40-chars (fallback ✅)
...
6. Turn controls: regenerate / edit-last / retry-with-model — needs `lakshx/rewind {turns}` in server.ts (splice session.history)
7. Permission policy: allow_always option per tool kind; durable ~/.lakshx/policy.json per workspace; audit JSONL receipts (~/.lakshx/audit/YYYY-MM.jsonl) — cheapest true enterprise differentiator. ✅ mode ladder UI
...
10. Usage/cost/context meter: loop.ts already gets usage — emit `lakshx/usage`; pricing.json table; footer meter `38% ctx · 12k↑ 3k↓ · $0.04`; warn at 80%
...
- Rules/memories: ~/.lakshx/rules.md + workspace .lakshx/rules.md injected into system prompt
