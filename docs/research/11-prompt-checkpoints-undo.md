...
**Why not rename nested `.git` dirs (Cline's approach, endorsed by doc 07's sketch):** researched fresh for this doc — Cline's `GitOperations.renameNestedGitRepos` temporarily renames nested `.git` → `.git_disabled` before staging, to stop git treating subdirectories as submodules during `git add -A`. This is **documented to corrupt real repositories** when interrupted mid-operation: `cline/cline#9590` ("infinite nested .git in node_modules, permanently renames .git to .git_disabled"), `#4385`/`#4388` ("Fix Checkpoint System Git Repository Corruption"), `#9631`. The failure mode is exactly the crash-window you'd expect: process killed between rename-out and rename-back leaves the user's or a dependency's `.git` renamed, silently breaking their real git tooling until someone notices.

**Koder's alternative — never touch any `.git`, real or nested, at all**: use git's **magic pathspec excludes** (supported since git 1.9) on every staging call instead of a rename step:
...
Combined with the `info/exclude` patterns above (which already exclude `node_modules` etc. so most nested-`.git`-bearing subtrees never get walked in the first place), this reaches the same practical outcome — nested repos aren't accidentally vendored into checkpoint history — **without a rename/restore step that can be interrupted mid-flight**. This is a strictly safer design than Cline's for the exact failure class its own issue tracker documents, and it removes an entire category of crash-recovery logic we'd otherwise need to write and test.

For `write_file`/`edit_file` (which always know their own single path, from `tools.ts:81-126`'s `input.path`), skip the tree-wide `add -A` entirely and `git add -- <path>` directly — narrower, faster, and structurally can't touch a nested repo since it's one explicit path. Only `bash` (which can touch arbitrary, unknown paths) needs the tree-wide scoped `add -A` above.
...
Two independent caps, mirroring `store.ts`'s existing `pruneSessions(keepNewest=200, maxAgeDays=60)` (`store.ts:104-128`) pattern:

- **Reachability-based**: call `git --git-dir="$GITDIR" reflog expire --expire=60.days --all && git --git-dir="$GITDIR" gc --prune=now --quiet` at server startup (same call site as the existing `pruneSessions()` at `server.ts:23`) — this ages out checkpoint commits older than 60 days *only once nothing references them*, matching the session-file cutoff so "can I undo this?" and "does the session file still exist to show it?" stay in sync by construction (once a session's `StoredSession` is pruned, its `promptId`s are unreachable from the UI anyway, so there's no correctness requirement to keep the underlying shadow commits either).
- **Size-based (unbounded-growth risk over a long session, called out in the task)**: `git gc` alone bounds pack bloat but not commit *count* in a very long-lived, frequently-checkpointed workspace. Track `$GITDIR` size at startup; if it exceeds **250 MB**, compact by creating a new orphan root at current HEAD (`git checkout --orphan tmp && git commit -m "checkpoint history compacted" && ` swap branches, `git gc --prune=now`) — this intentionally **destroys the ability to undo prompts older than the compaction point**, which is an acceptable, explicit tradeoff given it only fires past a size threshold that implies a very long session, and is logged as a `system` transcript event ("older undo history was compacted to bound disk usage") so it's never a silent data-loss surprise.
- No per-workspace commit-count cap beyond the above — the size/age caps are the actual constraints that matter; a count-based cap would just be a proxy for size and adds a second knob to tune for no benefit.
...
```
git --git-dir="$GITDIR" --work-tree="$WORKTREE" diff --quiet HEAD -- "$path"
```

Non-zero exit = the current on-disk content differs from the shadow repo's HEAD tree for that path. Since we commit after every mutating tool call (§2.3), shadow-HEAD for a given path is by definition "what the agent last wrote there" (or the baseline, if untouched since) — so a diff here means **something changed the file outside the checkpoint mechanism**, almost always the user editing it in the IDE between agent turns (the one gap: a manual edit made *between* two agent tool calls that both touch the same file gets silently folded into the next agent commit rather than flagged — documented as a known limitation below, not fixable without a file-system watcher, which is out of scope here).
